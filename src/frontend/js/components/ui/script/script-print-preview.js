...
const ScriptPrintPreview = React.createClass({
    ...
    render() {
        const { script, characters } = this.props;
        if (!characters)
        ...
        const titleNodes = tokens.titleTokens.map(function(token, i){
            return (
                <ScriptToken token={ token } characters={ characters } type='title' key={ i }/>
            )
            return null;
        });
        ...
        const scriptNodes = tokens.scriptTokens.map(function(token, i){
            return (
                <ScriptToken token={ token } characters={ characters } type='script' key={ i }/>
            )
        });
