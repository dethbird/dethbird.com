...
class WordpressData extends DataBase {
    ...
    public function getPosts($ids)
    {
        ...
        if(!$cache) {
            // echo $cacheKey; die();
            foreach ($ids as $id) {
                $response = $this->httpClient->get('http://blog.rishisatsangi.com/wp-json/wp/v2/posts/' . $id)->send();
                $response = json_decode($response->getBody(true));

                if($response->featured_image > 0) {
                    $mediaResponse = $this->httpClient->get('http://blog.rishisatsangi.com/wp-json/wp/v2/media/' . $response->featured_image)->send();
